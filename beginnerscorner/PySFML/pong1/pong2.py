...
# Window to Show the Game and Manage Key input
class windowmanager(object):
    def __init__(self,name):
        ...
        self._running = True
        # Frames per Scound Counterelements
        self._fpsCounter = 0
        self._counterTime = time.time()
        self.fps = 0
        self.fpsBox = pongTxtBox("0")
        self.fpsBox.xPos = 50
        self.fpsBox.yPos = 650                                       
    ...
    def show(self):
        while self._running:
            # Frames per scound showelemts
            self.fpsZahl = str(self._fpsCount())
            self._PongObjDict[0].text = self.fpsZahl
            # sending the Keypress and Mouseevents to the PongObjects
            self._eventmanager()
            # SFML Part for clearing, drawing Window
    ...
    def registerPongObj(self, pongObj):
        # a PongObject must be registered to be showen and get events
...
class pongBall(pongObj):
    ...
    def advance(self):
        if not self._gameover:
            ...
            if self.xPos >= 775:
                self._dx = -self._dx                                  
            if self.xPos <=90:  # Bug if here <= 0 not nice but funktional
                self._outofwindow = True
            if self.xPos <= 100:
                if self.yPos >= self._Balken1.yPos and self.yPos <= self._Balken1.yPos + 100:
...
class pongTxtBox(pongObj):
    ...
    # Hier muessen noch Propertys fuer Schrift und Groesse hin
